...
public class ShipmentOrderMainResource {
  ...
  @PutMapping("/shipment-order-main/edit/{id}")
  public ResponseEntity<ShipmentOrderMainDTO> updateShipmentOrderMain(
    ...
    return ResponseEntity.ok()
        .headers(HeaderUtil.createEntityUpdateAlert(
            applicationName, true, ENTITY_NAME, shipmentOrderMainDto.getId().toString()))
        .body(result);
  }

  @GetMapping("/shipment-order-main/orderDetails/{id}")
  public ResponseEntity<ShipmentOrderMainDTO> getShipmentOrderMainByIdOrderMain(
      @PathVariable Long id) {
    log.debug("REST request to get ShipmentOrderMain by id OrderMain");
    Optional<ShipmentOrderMainDTO> shipmentOrderMainDto
        = shipmentOrderMainService.findByOrderMainId(id);
    return ResponseUtil.wrapOrNotFound(shipmentOrderMainDto);
  }

  @GetMapping("/shipment-order-main/{id}")
